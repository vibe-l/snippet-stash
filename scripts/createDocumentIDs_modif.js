...
class DocumentIDGenerator {
  ...
  generateId(document, documentIndex, otherDocuments = [], usedIds = new Set()) {
    ...
    let finalId = idWords.map(w => w.word).join('_');
    
    // If this ID would be a duplicate, try to differentiate it
    if (usedIds.has(finalId)) {
      this.log(`\n=== DUPLICATE ID DETECTED: "${finalId}" ===`);
      finalId = this.differentiateId(document, documentIndex, idWords, wordPairs, otherDocuments, usedIds);
    }
  ...
  differentiateId(document, documentIndex, currentIdWords, allWordPairs, otherDocuments, usedIds) {
    this.log('Attempting to differentiate duplicate ID...');
    
    // Get the current ID positions to find gaps
    const usedPositions = new Set(currentIdWords.map(w => w.position));
    const usedLemmatized = new Set(currentIdWords.map(w => w.lemmatized));
    
    // Find words that could differentiate this document from others with the same base ID
    const candidateWords = [];
    
    for (const { original, lemmatized } of allWordPairs) {
      const wordData = allWordPairs.find(wp => wp.lemmatized === lemmatized);
      if (!wordData || usedLemmatized.has(lemmatized)) continue;
      
      const wordCount = this.docWordCounts.get(lemmatized) || 0;
      
      // Check if this word appears in other documents that might generate similar IDs
      let isUnique = true;
      for (const otherDoc of otherDocuments) {
        const otherWords = this.preprocessText(otherDoc, true);
        if (otherWords.some(w => w.lemmatized === lemmatized)) {
          isUnique = false;
          break;
        }
      }
      
      if (isUnique || wordCount <= this.maxMeanWordCount) {
        candidateWords.push({
          word: original,
          lemmatized,
          wordCount,
    ...
    // Sort candidates: unique words first, then by word count (lower is better), then by position
    ...
    this.log(`Found ${candidateWords.length} candidate differentiating words:`, 
             candidateWords.map(w => `${w.word}(${w.wordCount}, pos:${w.position}, unique:${w.isUnique})`));
    
    // Try adding each candidate word to create a unique ID
    for (const candidate of candidateWords) {
      const newIdWords = [...currentIdWords];
      
      // Find the correct insertion position based on document position
      let insertIndex = 0;
      for (let i = 0; i < newIdWords.length; i++) {
        if (newIdWords[i].position < candidate.position) {
          insertIndex = i + 1;
        } else {
          break;
        }
      ...
      newIdWords.splice(insertIndex, 0, candidate);
      const testId = newIdWords.map(w => w.word).join('_');
      
      this.log(`Testing ID with "${candidate.word}" inserted at position ${insertIndex}: "${testId}"`);
      
      if (!usedIds.has(testId)) {
        this.log(`Found unique ID: "${testId}"`);
        return testId;
      }
    }
    
    // If no differentiating word worked, fall back to numeric suffix
    this.log('No differentiating word found, falling back to numeric suffix');
    const baseId = currentIdWords.map(w => w.word).join('_');
    let counter = 2;
    let testId = `${baseId}_${counter}`;
  ...
  generateIds(documents, docWordCountPath = null) {
    ...
    for (let i = 0; i < documents.length; i++) {
      // Pass other documents and used IDs for duplicate detection
      const otherDocuments = documents.filter((_, index) => index !== i);
      const id = this.generateId(documents[i], i, otherDocuments, usedIds);
